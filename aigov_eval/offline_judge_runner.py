...
3. Generates schema-valid judge_output.json files
4. Does NOT touch live execution, Petri, or Inspect
...
def generate_judge_output(fixture_path: Path) -> dict[str, Any]:
    """
    Generate schema-valid judge output from a fixture file.

    Args:
        fixture_path: Path to calibration case fixture JSON file

    Returns:
        Judge output dictionary with schema-valid structure:
        {
...
def run_offline_judge(
    ...
    for fixture_path in fixture_files:
        try:
            ...
            judge_output = generate_judge_output(fixture_path)
            ...
            if verbose:
                ...
                print(f"     Verdict: {judge_output['verdict']}")
                print(f"     Signals: {', '.join(judge_output['signals']) if judge_output['signals'] else 'none'}")
